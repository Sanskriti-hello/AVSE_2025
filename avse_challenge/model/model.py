...
class SqueezeformerAVSE(nn.Module):
    """Optimized Squeezeformer AVSE for COG-MHEAR Challenge Structure"""
    ...
    def __init__(self, sample_rate=16000):
        ...
        # Linear decoder
        self.decoder = LinearDecoder(512, 257)
    ...
    def forward(self, mixture_audio, target_video):
        """
        Args:
            mixture_audio: [B, T] - Mixed audio signal
            target_video: [B, T_v, C, H, W] - Target speaker video
        Returns:
            enhanced_audio: [B, T] - Enhanced audio
        """
        # Audio processing
        mixture_spec = self.audio_to_spec(mixture_audio)  # [B, F, T_spec]
        audio_patches = self.audio_patch_embed(mixture_spec)  # [B, 512, T_patch]
        audio_features = audio_patches.transpose(1, 2)  # [B, T_patch, 512]
        audio_features = self.audio_pos_encoding(audio_features)
        
        # Process through Squeezeformer blocks
        for block in self.audio_blocks:
            audio_features = block(audio_features)
        
        # Video processing
        B, T_v, C, H, W = target_video.shape
        video_features = self.video_backbone(target_video.transpose(1, 2))  # [B, 128, T_v, 7, 7]
        video_features = video_features.permute(0, 2, 1, 3, 4)  # [B, T_v, 128, 7, 7]
        video_features = video_features.reshape(B, T_v, -1)  # [B, T_v, 128*7*7]
        video_features = self.video_transformer(video_features)  # [B, T_v, 512]
        
        # Cross-modal fusion
        fused_features = self.cross_attn(audio_features, video_features)
        
        # Generate enhanced spectrogram
        enhanced_spec = self.decoder(fused_features, mixture_spec)
    ...
    def spec_to_audio(self, spec_magnitude):
        """Convert spectrogram back to audio using overlap-add"""
        # Use original phase (simplified - could use learned phase)
...
class SqueezeformerBlock(nn.Module):
    def __init__(self, dim, heads, downsample=False):
        ...
        # MH-SA + FFN (MF block) or Conv + FFN (CF block)
        self.attention = nn.MultiheadAttention(dim, heads, batch_first=True)
        self.conv = nn.Conv1d(dim, dim, 31, padding=15, groups=dim)  # Depthwise conv
    ...
    def forward(self, x):
        ...
        # Convolution path (replace with conv for CF blocks)
        conv_x = x.transpose(1, 2)  # [B, T, C] -> [B, C, T]
        conv_out = self.conv(conv_x).transpose(1, 2)  # [B, C, T] -> [B, T, C]
        x = x + conv_out
...
class BidirectionalCrossAttention(nn.Module):
    def __init__(self, dim, heads, layers):
        super().__init__()
        self.layers = layers
        self.audio_to_video = nn.ModuleList([
            nn.MultiheadAttention(dim, heads, batch_first=True) for _ in range(layers)
        ])
        self.video_to_audio = nn.ModuleList([
            nn.MultiheadAttention(dim, heads, batch_first=True) for _ in range(layers)
        ])
...
class LinearDecoder(nn.Module):
    ...
    def forward(self, features, mixture_spec):
        # Generate mask
        mask = torch.sigmoid(self.layers(features))  # [B, T, F]
